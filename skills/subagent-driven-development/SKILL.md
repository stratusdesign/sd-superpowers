...
2. **Handle the report.** DONE → review. DONE_WITH_CONCERNS → read the concerns; correctness or scope concerns are addressed before review. NEEDS_CONTEXT → supply it, re-dispatch. BLOCKED → change something (context, model, task size, or escalate) — never force an unchanged retry.
...
5. **Complete.** Ledger line `Task <N>: complete (commits <base7>..<head7>, review clean)` (or `<K> parked`) — appended in the same message as your other bookkeeping — mark the todo, move on. Never advance past open, unparked Critical/Important findings.
