...
| P | Independent capability absent | Absence disclosed; workflow continues; no fabricated independent result | Degraded-independence disclosure `OBSERVED (incidental)`; actual absent-route detection **not exercised** (`STATIC CONTRACT`) | Probe N spontaneously disclosed its independent perspective was degraded and continued without fabricating a second-model result — but that was a probe framed as in-context self-review, NOT runtime detection of an absent route. The detect-absent-route-then-disclose path is required by the skill text but was never triggered by an actually-missing route |
