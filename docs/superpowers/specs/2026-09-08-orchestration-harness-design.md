...
4. **Work.** Seat duties per Spec A; statuses and done-claims per Spec C. The foreman never
   ...
   While assigned work needs handling, maintain a supported wait/check and consume its result.
   An idle/active flag alone does not establish completion; inspect the report/response.
   A promise to check later schedules nothing. Continue after completion unless a specific
   recorded operator decision blocks progress; no new watcher or scheduling service.
5. **Completion.** Done-claim appended → `review` → foreman commissions the matching reviewer
   ...
   (original purpose, approved scope/non-goals, artifacts and standing rubric) → findings
   dispositioned against that purpose: in-scope rework returns to the builder in the same
   ticket; disputed content goes to sa; material scope changes go to operator. Severity alone
   never authorizes a corrective ticket. Consider deleting/narrowing excessive requirements
   before adding infrastructure → acknowledgment → `done`. All
